...
## Langkah 3 — Import data leads yang sudah ada (opsional)
...
## Catatan keamanan
Dashboard Admin dilindungi password (lihat konstanta ADMIN_PASSWORD di
src/App.jsx). Ini hanya penghalang sisi-tampilan, bukan keamanan sungguhan,
karena aplikasi berjalan sepenuhnya di sisi klien. Kalau nanti perlu login
per pemasar dan hak akses berlapis, itu perlu ditambahkan lewat Supabase
Auth — tinggal bilang kalau mau dibantu langkah berikutnya.

## Langkah 7 — Tambah data nasabah inaktif (opsional, untuk menu "Data")
1. Di SQL Editor, jalankan isi file `supabase/schema_customers_inactive.sql`.
   Ini membuat tabel baru `customers_inactive` — tabel `leads` tidak disentuh sama sekali.
2. Buka Table Editor > `customers_inactive` > Insert > Import data from CSV,
   lalu unggah `supabase/customers_inactive.csv` (1.173 baris data nasabah inaktif).
3. Menu "Data" di aplikasi akan otomatis menampilkan seluruh data ini, lengkap dengan
   filter dan tombol Follow Up yang memasukkan hasilnya sebagai prospek baru ke
   Daftar Prospek (sumber: "Nasabah Inaktif").
