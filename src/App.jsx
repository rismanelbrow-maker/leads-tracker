...
function parseFollowUps(leads) {
  ...
  leads.forEach((l) => {
    ...
    if (lines.length === 0) {
      rows.push({ id: `${l.id}-0`, tanggal: l.tanggal, nama: l.nama, produk: l.produk, kategori, unit: l.unit, keterangan: l.keterangan, hasil: "-", status: l.status });
      return;
    }
...
function FollowUpPanel({ leads }) {
  ...
  const exportPng = () => {
    const cols = [
      { key: "tanggal", label: "Tanggal follow up", width: 110 },
      { key: "nama", label: "Nama", width: 140 },
      { key: "produk", label: "Produk", width: 170 },
      { key: "unit", label: "Unit kerja", width: 120 },
      { key: "keterangan", label: "Keterangan", width: 110 },
      { key: "hasil", label: "Hasil follow up", width: 260 },
      { key: "status", label: "Status", width: 95 },
    ];
    const padding = 24;
    const rowH = 28;
...
function computePeriodRange(period) {
  ...
  const spanDays = period === "mingguan" ? 6 : 29;
  const fromDate = new Date(today);
  fromDate.setDate(fromDate.getDate() - spanDays);
...
function ReportPanel({ leads }) {
  ...
  const { fromStr, toStr } = computePeriodRange(period);
  ...
  const periodLabel = period === "mingguan" ? "Mingguan" : "Bulanan";

  const handleDownload = () => {
    const html = buildReportHtml({ periodLabel, fromStr, toStr, data });
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 16 }}>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setPeriod("mingguan")} style={{ padding: "8px 16px", borderRadius: 999, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500, background: period === "mingguan" ? "#0A5C36" : "#F1EFE8", color: period === "mingguan" ? "#fff" : "#5F5E5A" }}>Mingguan</button>
          <button onClick={() => setPeriod("bulanan")} style={{ padding: "8px 16px", borderRadius: 999, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500, background: period === "bulanan" ? "#0A5C36" : "#F1EFE8", color: period === "bulanan" ? "#fff" : "#5F5E5A" }}>Bulanan</button>
        </div>
      ...
      <div style={{ fontSize: 12, color: "#888780", marginBottom: 16 }}>
        Periode: {formatTanggalPanjang(fromStr)} s/d {formatTanggalPanjang(toStr)} — {data.total} leads tercakup. Tombol unduh akan membuka dialog cetak browser; pilih tujuan cetak <strong>"Simpan sebagai PDF"</strong>.
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: "#fff", borderRadius: 12, padding: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 10 }}>Perbandingan kategori produk</div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead><tr style={{ textAlign: "left", color: "#888780" }}><th style={{ padding: "4px 6px" }}>Kategori</th><th style={{ padding: "4px 6px" }}>Total</th><th style={{ padding: "4px 6px" }}>Closing</th></tr></thead>
            <tbody>{data.categoryRows.map((r) => (
              <tr key={r.kategori} style={{ borderTop: "1px solid #F1EFE8" }}>
                <td style={{ padding: "6px" }}>{r.kategori}</td><td style={{ padding: "6px" }}>{r.total}</td><td style={{ padding: "6px" }}>{r.closing} ({r.pct}%)</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
        <div style={{ background: "#fff", borderRadius: 12, padding: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 10 }}>Leaderboard pemasar (top 10)</div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead><tr style={{ textAlign: "left", color: "#888780" }}><th style={{ padding: "4px 6px" }}>Pemasar</th><th style={{ padding: "4px 6px" }}>Jumlah leads</th></tr></thead>
            <tbody>{data.pemasarRows.map((r) => (
              <tr key={r.nama} style={{ borderTop: "1px solid #F1EFE8" }}>
                <td style={{ padding: "6px" }}>{r.nama}</td><td style={{ padding: "6px" }}>{r.jumlah}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
