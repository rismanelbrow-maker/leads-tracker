...
function AllLeadsPanel({ leads, onUpdate }) {
  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", overflow: "auto", marginBottom: 20 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ textAlign: "left", color: "#888780", background: "#F8F9FA" }}>
              ...
              <th style={{ padding: "8px 12px" }}>Pemasar</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: 20, textAlign: "center", color: "#888780" }}>Tidak ada prospek yang cocok.</td></tr>
            ) : filtered.map((l) => (
              <tr key={l.id} style={{ borderTop: "1px solid #F1EFE8" }}>
      ...
      {selected && (
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", maxWidth: 720 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <strong style={{ fontSize: 15 }}>Edit tindak lanjut — {selected.nama}</strong>
...
function LeaderboardPanel({ leads }) {
  const byUnit = {}; const byPemasar = {};
  leads.forEach((l) => { byUnit[l.unit] = (byUnit[l.unit] || 0) + 1; byPemasar[l.pemasar] = (byPemasar[l.pemasar] || 0) + 1; });
  const unitData = Object.entries(byUnit).map(([name, total]) => ({ name, total })).sort((a, b) => b.total - a.total);
  const pemasarData = Object.entries(byPemasar).map(([name, total]) => ({ name, total })).sort((a, b) => b.total - a.total);
...
function AdminDashboard({ leads, onUpdate }) {
  ...
  return (
    <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
      <div style={{ width: 220, flexShrink: 0, background: "#fff", borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", padding: 10, position: "sticky", top: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: "#888780", textTransform: "uppercase", letterSpacing: 0.4, padding: "8px 10px 6px" }}>Menu</div>
        {tabs.map((t) => {
          const Icon = t.icon;
          ...
          return (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              ...
              padding: "10px 12px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13.5, fontWeight: 500,
