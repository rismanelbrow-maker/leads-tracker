...
function ReportPanel({ leads }) {
  ...
  const handleDownloadPng = () => {
    const sections = [
      ...
      { title: "Perbandingan kategori produk", rows: data.categoryRows, labelKey: "kategori", valueKey: "total", color: "#0A5C36" },
      { title: "Distribusi status leads", rows: data.statusRows, labelKey: "status", valueKey: "jumlah", color: "#378ADD" },
    ...
    sections.forEach((s) => { sectionsHeight += sectionTitleH + Math.max(s.rows.length, 1) * rowH + sectionGap; });

    const canvasWidth = width + padding * 2;
    const canvasHeight = headerH + kpiH + sectionsHeight + padding;
    ...
    ctx.fillText(`Laporan ${presetLabel} — Pegadaian Mikro & Emas`, padding, 28);
    ctx.fillStyle = "#5F5E5A";
    ctx.font = "12px Arial, sans-serif";
    const genAt = new Date().toLocaleString("id-ID", { dateStyle: "long", timeStyle: "short" });
    ...
    const link = document.createElement("a");
    link.download = `laporan-${presetLabel.toLowerCase().replace(/\s+/g, "-")}-${todayStr()}.png`;
    link.href = canvas.toDataURL("image/png");
    link.click();
  };
    ...
    <div>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={handleDownloadPng} style={{ display: "flex", alignItems: "center", gap: 6, background: "#fff", color: "#3d3d3a", border: "1px solid #D3D1C7", borderRadius: 8, padding: "9px 16px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
      ...
      <div style={{ fontSize: 12, color: "#888780", marginBottom: 16 }}>
        Periode {presetLabel.toLowerCase()}: {formatTanggalPanjang(fromStr)} s/d {formatTanggalPanjang(toStr)} — {data.total} leads tercakup. Tombol unduh akan membuka dialog cetak (pilih "Simpan sebagai PDF") atau menyimpan langsung sebagai gambar PNG.
      </div>
