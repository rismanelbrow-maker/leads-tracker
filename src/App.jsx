...
import {
  ...
  List, Coins, Gem, Trophy, ChevronLeft, ChevronRight, ArrowUp, ArrowDown, History, Layers, MoreVertical, FileText, FileDown
} from "lucide-react";
...
function UpdateLeadPanel({ leads, onUpdate }) {
  ...
  const handleSave = (e) => {
    ...
    const appended = note.trim() ? `${selected.catatan}\n${stamp} [${selected.pemasar}]: ${note.trim()}` : selected.catatan;
...
function ExportMenu({ onExportPng, onExportCsv }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);
...
function AllLeadsPanel({ leads, onUpdate, hideKategori }) {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    const stamp = todayStr();
    const appended = note.trim() ? `${selected.catatan}\n${stamp} [Admin]: ${note.trim()}` : selected.catatan;
    onUpdate(selected.id, { keterangan: ket, status, catatan: appended });
    setSaved(true);
    setNote("");
  };
  ...
  leads.forEach((l) => {
    ...
    followUpLines.forEach((line, idx) => {
      ...
      rows.push({
        ...
        diinputOleh: m && m[2] ? m[2] : "Tidak diketahui",
        nama: l.nama,
        produk: l.produk,
        kategori,
        unit: l.unit,
...
const FOLLOWUP_EMPTY_FILTERS = { tanggal: new Set(), nama: new Set(), produk: new Set(), unit: new Set(), diinputOleh: new Set(), keterangan: new Set(), status: new Set() };
...
function FollowUpPanel({ leads }) {
  ...
  const options = useMemo(() => ({
    ...
    diinputOleh: [...new Set(rows.map((r) => r.diinputOleh))].sort(),
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return matchQuery
        ...
        && matchField("diinputOleh", r.diinputOleh)
  ...
  const exportPng = () => {
    const cols = [
      ...
      { key: "diinputOleh", label: "Diinput oleh", width: 110 },
    ...
    if (sorted.length === 0) {
      ...
      sorted.forEach((r, i) => {
        ...
        ctx.fillStyle = "#2C2C2A";
        const vals = [r.tanggal, r.nama, r.produk, r.unit, r.diinputOleh, r.keterangan, r.hasil, r.status];
        let cx = padding;
        cols.forEach((c, ci) => {
          let text = String(vals[ci] ?? "");
          const maxChars = Math.max(4, Math.floor((c.width - 12) / 5.6));
  ...
  const exportCsv = () => {
    const headers = ["tanggal_follow_up", "nama", "produk", "unit_kerja", "diinput_oleh", "keterangan", "hasil_follow_up", "status"];
    const rows_ = sorted.map((r) => [r.tanggal, r.nama, r.produk, r.unit, r.diinputOleh, r.keterangan, r.hasil, r.status]
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ textAlign: "left", color: "#888780", background: "#F8F9FA" }}>
              ...
              <th style={{ padding: "8px 12px" }}>
                <FilterDropdown label="Diinput oleh" fieldKey="diinputOleh" options={options.diinputOleh} selected={filters.diinputOleh} onChange={setFilter("diinputOleh")} sortState={sortState} onSort={handleSort} variant="header" />
              </th>
              <th style={{ padding: "8px 12px" }}>
          ...
          <tbody>
            {paged.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: 20, textAlign: "center", color: "#888780" }}>Tidak ada catatan follow up yang cocok.</td></tr>
            ) : paged.map((r) => (
              <tr key={r.id} style={{ borderTop: "1px solid #F1EFE8" }}>
                ...
                <td style={{ padding: "8px 12px" }}>{r.diinputOleh}</td>
...
function buildReportData(leads) {
  ...
  return { total, closing, activePipeline, conversion, categoryRows, statusRows, pemasarRows, unitRows, sumberRows };
}

function buildReportHtml({ periodLabel, fromStr, toStr, data }) {
...
<body>
  <h1>Laporan ${periodLabel} — Pegadaian Mikro &amp; Emas Leads Tracker</h1>
...
function ReportPanel({ leads }) {
  ...
  const [presetLabel, setPresetLabel] = useState("Minggu ini");

  const applyPreset = (label) => {
    const today = new Date();
    if (label === "Minggu ini") {
      setFromStr(startOfWeek(today).toISOString().slice(0, 10));
      setToStr(todayStrNow);
    } else if (label === "Bulan ini") {
      setFromStr(new Date(today.getFullYear(), today.getMonth(), 1).toISOString().slice(0, 10));
      setToStr(todayStrNow);
  ...
  const handleDownload = () => {
    const html = buildReportHtml({ periodLabel: presetLabel, fromStr, toStr, data });
  ...
  const presets = ["Minggu ini", "Bulan ini", "7 hari terakhir", "30 hari terakhir"];
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 10 }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 14, alignItems: "flex-end" }}>
          <div>
            ...
            <input type="date" value={fromStr} max={toStr} onChange={(e) => { setFromStr(e.target.value); setPresetLabel("Kustom"); }} style={{ ...inputStyle, padding: "7px 10px" }} />
          </div>
          <div>
            <div style={{ fontSize: 11.5, color: "#888780", marginBottom: 4 }}>Sampai tanggal</div>
            <input type="date" value={toStr} min={fromStr} max={todayStrNow} onChange={(e) => { setToStr(e.target.value); setPresetLabel("Kustom"); }} style={{ ...inputStyle, padding: "7px 10px" }} />
          </div>
        </div>
        <button onClick={handleDownload} style={{ display: "flex", alignItems: "center", gap: 6, background: "#0A5C36", color: "#fff", border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
        {presets.map((p) => (
          <button key={p} onClick={() => applyPreset(p)} style={{ padding: "6px 14px", borderRadius: 999, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 500, background: presetLabel === p ? "#0A5C36" : "#F1EFE8", color: presetLabel === p ? "#fff" : "#5F5E5A" }}>
            {p}
          </button>
        ))}
      </div>
      ...
      <div style={{ fontSize: 12, color: "#888780", marginBottom: 16 }}>
        Periode {presetLabel.toLowerCase()}: {formatTanggalPanjang(fromStr)} s/d {formatTanggalPanjang(toStr)} — {data.total} leads tercakup. Tombol unduh akan membuka dialog cetak browser; pilih tujuan cetak <strong>"Simpan sebagai PDF"</strong>.
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        <KpiCard icon={<Users size={16} color="#0A5C36" />} label="Total leads" value={data.total} sub="Periode terpilih" color="#0A5C36" />
        <KpiCard icon={<CheckCircle2 size={16} color="#1D9E75" />} label="Leads cair" value={data.closing} sub="Status: DISBURSED" color="#1D9E75" />
        <KpiCard icon={<TrendingUp size={16} color="#A37F15" />} label="Konversi" value={`${data.conversion}%`} sub="Leads cair vs total" color="#A37F15" />
        <KpiCard icon={<Clock size={16} color="#378ADD" />} label="Pipeline aktif" value={data.activePipeline} sub="Masih tindak lanjut" color="#378ADD" />
      </div>
...
function AdminDashboard({ leads, onUpdate }) {
  ...
  const tabs = [
    ...
    { key: "Laporan", label: "Laporan (PDF)", icon: FileDown, color: "#B3436B" },
  ];
